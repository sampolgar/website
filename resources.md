...
`const header = document.createElement('h1') const headerText = document.createTextNode('Hello World'); header.appendChild(headerText)`

### React + JSX (Declerative)

- add script type = text/jsx
  `const header = document.getElementById('header') ReactDOM.render(<h1>Hello, World</h1>, header) `
...
- Without
  `` function getFullName(nameObj) { return `${nameObj.firstName} ${nameObj.lastName}` }``
- With
...
# React Hooks
- Hooks let you hook into a component's rendering, use the useState() Hook.
## rendering
<!-- render whatevers on the left e.g. Header into the right (app) -->
`<div id="app"></div>`
